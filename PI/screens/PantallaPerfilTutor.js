...
export default function PantallaDetalleTutoria({ route, navigation }) {
  const { tutoria } = route.params || {};
  const usuarioActual = getUsuarioActual();

  // -------------------------------------------------------
  // VALIDACIÓN: SI NO HAY TUTORÍA
  // -------------------------------------------------------
  if (!tutoria) {
    return (
  ...
  // -------------------------------------------------------
  // FUNCIÓN: MANEJAR UNIRSE A LA CLASE
  // -------------------------------------------------------
  ...
  // -------------------------------------------------------
  // FUNCIÓN: MANEJAR VER UBICACIÓN
  // -------------------------------------------------------
  const manejarVerUbicacion = () => {
    if (!tutoria.ubicacion) {
      Alert.alert('Sin ubicación', 'Esta clase no tiene una ubicación registrada.');
      return;
    }
        ...
        <View style={styles.perfilInfo}>
          <View style={styles.avatarContainer}>
            <View style={styles.avatarGrande}>
              <Text style={styles.avatarGrandeText}>
                {tutoria.tutorNombre
                  ? tutoria.tutorNombre.substring(0, 2).toUpperCase()
                  : 'TU'}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.datosTutor}>
            ...
            <View style={styles.ratingContainer}>
              ...
              <Text style={styles.ratingText}>4.9 (15 reseñas)</Text>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  link: {
    color: '#8B5CF6', marginTop: 10, fontWeight: 'bold'
  },

  scrollView: { flex: 1 },

  header: { height: 120, position: 'relative' },
  ...
  perfilInfo: { paddingHorizontal: 20, marginTop: -40 },
  ...
  avatarGrande: {
    ...
    backgroundColor: '#8B5CF6', justifyContent: 'center',
    alignItems: 'center', borderWidth: 4, borderColor: 'white'
  },
  ...
  especialidadTutor: {
    fontSize: 16, color: '#6B7280', marginBottom: 8
  },
  ...
  botonAccionSecundario: {
    ...
    borderWidth: 1, borderColor: '#8B5CF6', gap: 8
  },
  ...
  seccionTitulo: {
    fontSize: 18, fontWeight: 'bold', color: '#1F2937', marginBottom: 12
  },
  ...
  infoValue: {
    fontSize: 14, color: '#1F2937', fontWeight: '600'
  },

  infoValuePrecio: {
    fontSize: 14, color: '#8B5CF6', fontWeight: '700'
  }
});
